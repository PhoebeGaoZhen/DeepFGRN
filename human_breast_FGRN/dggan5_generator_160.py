...
def generator_CNN2_model():
    ...
    model.add(tf.keras.layers.Dense(40*16, use_bias=False))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.LeakyReLU())
    # (16,16)
    # (32,16)
    model.add(tf.keras.layers.Reshape((40,16)))
...
def generator_CNN1_model():
    ...
    model.add(tf.keras.layers.Dense(80*16, use_bias=False))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.LeakyReLU())
    # (64, 16)
    model.add(tf.keras.layers.Reshape((80, 16)))
