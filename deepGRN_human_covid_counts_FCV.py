...
from DeepFGRN import three_utils_2_single, dggan_embedding_param
from DeepFGRN import corrresnet_pred_224 as corrresnet_pred
...
path_network_ids = 'traindataHuman\\final_GRN_ids\\new_GRN_COVID_GEN_counts_genename_ids.tsv'
path_node = 'traindataHuman\\final_genelist_txt\\new_exp_COVID_GEN_counts_genename_ids.txt'

output_directory = '.\\output_directory\\'
if not os.path.isdir(output_directory):
    os.makedirs(output_directory)
logTime = time.strftime("%Y-%m-%d-%H%M%S", time.localtime())
network_dict_name = 'COVID_counts_'  + str(dim_net) + '_' + logTime
save_index_path = '.\\three_results\\1019\\'
