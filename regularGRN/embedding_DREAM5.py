...
class Model():
    ...
    def __init__(self, pathnetwork, pathnode):
        ...
        self.graph, self.n_node, self.node_list, self.node_list_s, self.egs = utils.read_graph_DREAM4(pathnetwork,pathnode)
        print()
    ...
    def train(self):
        for epoch in range(config.n_epoch):
            ...
            for d_epoch in range(config.d_epoch):
                dis_loss, dis_pos_loss, dis_neg_loss, dis_cnt, avg_dis_loss = self.train_dis(dis_loss, dis_pos_loss,
                                                                                             dis_neg_loss, dis_cnt)
    ...
    def prepare_data_for_d(self, index, egs):
        ...
        fake_node_embedding = []
        noise_embedding = np.random.normal(0.0, config.sig,
                                           (2, len(pos_node_ids), config.n_emb))
